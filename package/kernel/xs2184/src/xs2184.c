...
{
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_READ_I2C_BLOCK | I2C_FUNC_SMBUS_WRITE_I2C_BLOCK))
		return -ENODEV;

	struct gpio_desc *gpio_en = devm_gpiod_get_optional(&client->dev, "enable", GPIOD_OUT_HIGH);
	if (IS_ERR(gpio_en))
...
module_i2c_driver(xs2184_driver);
